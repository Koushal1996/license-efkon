...
export class ForgotPwdComponent implements OnInit {
  ...
  get newPwdsGrp() {
    return this.forgotPwdForm.get("pwds").get("newPwds");
  }
  ...
  passwordMatchValidator(subForm: FormGroup) {
    return subForm.get("newPassword").value ===
      subForm.get("newPasswordConfirm").value
      ? null
      : { mismatch: true };
  }
  ...
  generateTempPwd() {
    ...
    this.authService
      .forgotPassword(this.forgotPwdForm.value.username)
      .subscribe(
        (res: any) => {
          swal(
            "A password has been generated and sent to your email. Please use that to reset password"
          );
          this.extendForm();
  ...
  submitNewPassword() {
    ...
    this.authService.resetPassword(payLoad).subscribe(
      (res: any) => {
        swal("Password reset successfull. Please login with new password.");
        this.requesting = false;
